...
${JSON.stringify(searchResults.slice(0, 10), null, 2)}`
    ...
    return { noa_url: null, fl_url: null, ul_url: null, installation_guide_url: null, pe_evaluation_url: null, source_website: null };
  } catch (error) {
    ...
    for (const product of sortedProducts) {
      try {
        ...
        if (foundDocs.installation_guide_url) {
          const localUrl = await downloadAndStorePdf(supabase, foundDocs.installation_guide_url, product.id, 'installation_guide');
          if (localUrl) {
        ...
        if (foundDocs.pe_evaluation_url) {
          const localUrl = await downloadAndStorePdf(supabase, foundDocs.pe_evaluation_url, product.id, 'pe_evaluation');
          if (localUrl) {
