...
const PDF_URL_PATTERNS = {
  ...
  miamidade: (noaNumber: string) => {
    ...
    return `https://www.miamidade.gov/building/library/productcontrol/noa/${cleaned}.pdf`;
  },
...
serve(async (req) => {
  ...
  try {
    ...
    if (pdfBytes && pdfBytes.length > 1000) {
      ...
      const { error: uploadError } = await supabase.storage.from("product-approvals").upload(storagePath, pdfBytes, {
      ...
      const { error: updateError } = await supabase.from("product_approvals").update(updateData).eq("id", productId);
