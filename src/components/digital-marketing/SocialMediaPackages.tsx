      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-pink-100 rounded-full px-4 py-2 mb-4">
            ...
            <span className="text-pink-600 text-sm font-medium">Social Media Management</span>
          </div>
          ...
          {socialPackages.map((pkg, index) => (
            ...
            >
              ...
              <CardHeader className="text-center pb-4">
                <Badge variant="outline" className="w-fit mx-auto mb-3 border-slate-300">
                  {pkg.platforms}
                </Badge>
                ...
                <ul className="space-y-3">
                  {pkg.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <Check className={`h-5 w-5 mt-0.5 flex-shrink-0 ${pkg.popular ? 'text-pink-500' : 'text-green-500'}`} />
        ...
        <div className="bg-slate-100 rounded-2xl p-8">
          ...
          <div className="flex flex-wrap gap-4">
            {addOns.map((addon, index) => (
              <div key={index} className="bg-white rounded-lg px-4 py-3 flex items-center gap-3 border border-slate-200">
