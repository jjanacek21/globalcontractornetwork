...
import type { RoofFacet, RoofEdge, EdgeType } from "./types";
...
export function FacetListPanel({
  facets, edges, selectedFacetId, onSelectFacet, onUpdateFacet, onDeleteFacet,
}: FacetListPanelProps) {
  // Edge totals
        ...
        <CardContent className="p-2 space-y-1.5">
          {facets.length === 0 && (
            <p className="text-xs text-muted-foreground text-center py-4">
              Draw facets on the map to begin measuring.
            </p>
          )}
          ...
          {facets.map((facet) => {
            ...
            return (
              <div
                ...
                onClick={() => onSelectFacet(isSelected ? null : facet.id)}
              >
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <Input
                    value={facet.name}
                    onChange={(e) => onUpdateFacet(facet.id, { name: e.target.value })}
                  ...
                  <Button
                    ...
                    onClick={(e) => { e.stopPropagation(); onDeleteFacet(facet.id); }}
                  >
                ...
                <div className="flex gap-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => onUpdateFacet(facet.id, { wastePercent: parseInt(v) })}
                  >
                    <SelectTrigger
                      className="h-6 text-[10px] w-[72px]"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <SelectValue />
                    </SelectTrigger>
