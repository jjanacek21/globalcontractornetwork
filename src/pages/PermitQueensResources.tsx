...
export default function PermitQueensResources() {
  ...
  const getResourceIcon = (type: string) => {
    switch (type) {
      ...
      case 'checklist': return <CheckSquare className="h-5 w-5 text-green-600" />;
  ...
  const getTypeBadge = (type: string) => {
    const colors: Record<string, string> = {
      ...
      checklist: 'bg-green-500/10 text-green-600',
      ...
      <header className="border-b bg-card">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-xl">Permit Resource Library</span>
          </div>
        </div>
      </header>
