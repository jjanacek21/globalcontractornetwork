 ...
 *
 * The brand animation ships as an MP4 with the logo sitting on a solid black
 * plate. `mix-blend-mode: screen` only hides that plate over dark surfaces, so
 * the black box reappears in light theme.
 *
 * This draws every frame to a canvas and turns near-black pixels into real
 * transparency, so only the mark, the lettering and the glow survive — on any
 * background, in any theme.
 */

/** Below this luma a pixel is plate; above it the pixel is fully opaque. */
const LO = 0.045;
const HI = 0.3;
/** Processing width — full-size frames cost more than the result shows. */
const MAX_W = 620;
...
export function mountLumaLogo(
  ...
  canvas.setAttribute("aria-label", target.alt || "Global Contractor Network");
  ...
  function frame() {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      const g = d[i + 1];
      const b = d[i + 2];
      // Perceptual luma, normalised. Black plate -> 0, mark/glow -> high.
